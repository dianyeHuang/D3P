'''
Script: testing_d3p_cm.py
Description:
    Deploy and evaluate a distilled policy on RLBench tasks, logging rich diagnostics
...
def select_timesteps(timelist, num):
    """
    nums: list of integers (e.g., [0, 1, ..., 29])
    num: number of elements in the final output list (including head & tail)
    
    Returns:
        A sublist of length 'num', sorted from largest to smallest.
    """
    n = len(timelist)
    if num > n:
        raise ValueError("num cannot be larger than the list length")
    # Compute step size (floating division for even spacing)
    step = (n - 1) / (num - 1)
    # Select evenly spaced indices
    selected = [timelist[round(i * step)] for i in range(num)]
    # Return in descending order
    return selected

from utils.d3p_policy_module_utils import D3P
def policy_inference(policy:D3P, qpos, image, num_infer_steps=3, num_errsamples=7, ret_generr=False):
    # compute condition
    vision_dict = { 
        'front_rgb': image[:, 0, ...],
        'wrist_rgb': image[:, 1, ...],
    }
    vis_enc = policy.obs_encoder(vision_dict) # (batch_size, dim)
    fuse_enc = policy.fuse(qpos, vis_enc) # fused feature
    latent_act = policy.dko.get_latnet_act(vis_enc)
    
    # get actions
    # -get fuse_acts
    # set timesteps
    policy.noise_scheduler.set_timesteps(num_infer_steps)
    timesteps = policy.noise_scheduler.timesteps.to(qpos.device)

    # prepare extra kwargs for the scheduler step, since not all schedulers have the same signature
    # eta (η) is only used with the DDIMScheduler, and between [0, 1]
    batch_size = fuse_enc.shape[0]
    traj = torch.randn(size=(batch_size, policy.pred_seqlen, policy.action_dim), device=fuse_enc.device)
    for i, t in enumerate(timesteps):
        # latents = policy.noise_scheduler.scale_model_input(latents, t)
        traj = policy.diffusion_model(
            sample=traj,
            timestep=t,
            local_cond=None,
    ...
    # - get vis_acts
    batch_size = latent_act.shape[0]
    traj = torch.randn(size=(batch_size, policy.pred_seqlen, policy.action_dim), device=latent_act.device)
    for i, t in enumerate(timesteps):
        # latents = policy.noise_scheduler.scale_model_input(latents, t)
        traj = policy.diffusion_model(
            sample=traj,
    ...
    if not ret_generr:
        ...
        fuse_err = get_generative_errs(policy, fuse_enc, fuse_acts, num_infer_steps, num_samples=num_errsamples)
        vis_err  = get_generative_errs(policy, latent_act, vis_acts, num_infer_steps, num_samples=num_errsamples)
        # print('fuse err: ', fuse_err, '. vis_err: ', vis_err)
        return {
...
def get_generative_errs(policy:D3P, global_cond:torch.Tensor, acts:torch.Tensor, num_infer_steps:int, num_samples:int):
        '''
        ...
        repeat_factors = [1]*global_cond.ndimension()
        repeat_factors[0] = num_samples
        global_cond = global_cond.repeat(*repeat_factors)
        ...
        timesteps = torch.randint(
            0, num_infer_steps, (batch_size, ), device=acts.device, dtype=torch.long
        ).long()
        noisy_traj = policy.noise_scheduler.add_noise(acts, noise, timesteps)
        pred_acts = policy.diffusion_model(
            sample=noisy_traj,
            timestep=timesteps,
            local_cond=None,
        ...
        loss = F.mse_loss(pred_acts, target=acts, reduction='mean')
        return loss
        ...
        with torch.inference_mode():
            for ts in range(episode_len): # max_episode_len defines the maximum steps allowed to complete the task
                ...
                if ts % query_freq == 0:
                    ...
                    act_dict = policy_inference(policy, obs_propri, obs_img, ret_generr=True)
                ...
                # print('act.numpy().tolist(): ', act[0].numpy().tolist())
...
from config_const import check_cfg, RunBuilder, act_dim, chunking_size, distill_cfg
...
from utils.consistency_utils import CTMConditionalUnet1D
from utils.baseline_utils import DiffusionPolicy
from utils.d3p_cm_utils import load_d3p_cm_diffmodel
def load_and_update_policy(policy:DiffusionPolicy, ckpt_path) -> Tuple[DiffusionPolicy, CTMConditionalUnet1D]:
    # load student diffusion model
    diffusion_model, ddim_scheduler = load_d3p_cm_diffmodel(ckpt_path=ckpt_path, device=device)
    policy.diffusion_model = diffusion_model
...
params = OrderedDict(
    ...
    query_freq  = check_cfg.query_freq,
...
if __name__ == '__main__':
    ...
    for run in tqdm(RunBuilder.get_runs(params)):
        if 'DKODPolicy' in run.method: run.query_freq = d3p_query
        ...
        # student_cfg = Config_distill[run.method]['policy_config']['diffmodel_config']
        ckpt_dir = distill_cfg.model_dir[run.task][run.method]
        policy = load_and_update_policy(
            policy=policy, # student_cfg=student_cfg, 
            ckpt_path=os.path.join(ckpt_dir, f'last_{run.method}_{seed}.pth')
        )
        set_seed(seed)
